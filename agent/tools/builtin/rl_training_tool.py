...
class RLTrainingTool(Tool):
    ...
    def __init__(self):
        ...
        # 检查TRL是否可用
        try:
            from hello_agents.rl.utils import check_trl_installation
            self.trl_available = check_trl_installation()
        except ImportError:
    ...
    def _handle_load_dataset(self, parameters: Dict[str, Any]) -> str:
        """处理数据集加载操作"""
        from hello_agents.rl.datasets import create_sft_dataset, create_rl_dataset
    ...
    def _handle_evaluate(self, parameters: Dict[str, Any]) -> str:
        """处理模型评估操作"""
        try:
            from hello_agents.rl.datasets import create_rl_dataset
            from hello_agents.rl.rewards import create_accuracy_reward, evaluate_rewards
    ...
    def _train_sft(
        ...
        from hello_agents.rl.datasets import create_sft_dataset
